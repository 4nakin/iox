...
play.init = function(a) {
	...
	play.map = function(a) {
		for (var b = [], c = 0; 15 > c; c++) for (var d = 0; 15 > d; d++) {
			var e = a[d][c];
...
play.mapShow = function(a) {
	for (var b = 0; 15 > b; b++) for (var c = 0; 15 > c; c++) {
		var d = a[c][b];
		if (d) {
			var e = -1 == d ? 0 : d;
...
play.regret = function() {
	...
	com.childList.pop(),
	play.map[a[1]][a[0]] = 0,
	a = play.pace[play.pace.length - 1],
	com.showPane(a[0], a[1]),
	com.show(),
	play.regretCount--,
	0 === play.regretCount && (com.get("regretBtn").style.opacity = .5),
	com.get("regretBtn").innerHTML = "悔   棋(" + play.regretCount + ")"
},
...
play.isWin = function(a, b) {
	function c(a) {
		for (var b = 0,
		c = 0,
		d = a.length; d > c; c++) if (a[c] === play.my) {
			if (b++, 5 == b) return ! 0
		} else b = 0;
		return ! 1
	}
