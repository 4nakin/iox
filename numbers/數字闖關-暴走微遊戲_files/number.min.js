	...
	var b = a.GameHomeScene = Hilo.Class.create({
		...
		initBtn: function() {
			...
			function(c) {
				...
				b.visible = !1,
				document.getElementById("moregame").style.display = "none",
				_czc.push(["_trackEvent", "點擊", "關卡選擇按鈕"])
			}.bind(b))
		}
	})
} (window.Game),
...
function(a) {
	var b = a.TipScene = Hilo.Class.create({
		...
		starY: 110,
		textInfo: {
			state: ["恭喜通關", "闖關失敗", "遊戲暫停", "關卡結束", "遊戲規則"],
			text: ["用時: ", "萌萌噠,再來一次吧!", "不要離開太久哦！", "恭喜你丫的,通關了", "點擊圖中方塊，該方塊及其相鄰的方塊數位減1，在規定時間內將所有的方塊都消為0，遊戲通關。"]
		},
		levelColor: "#fff",
		textColor: "#179446",
		constructor: function(a) {
		...
		initBtn: function() {
			...
			function(d) {
				...
				_czc.push(["_trackEvent", "點擊", "新手提示按鈕"])
			}.bind(this))
		}
	})
} (window.Game),
	...
	var b = a.LevelScene = Hilo.Class.create({
		...
		initBtn: function() {
			...
			function(e) {
				...
				_czc.push(["_trackEvent", "點擊", "上一頁按鈕", d - b.pLevelNum])
			}.bind(this)),
			...
			function(e) {
				...
				c.removeChild(b),
				a.creatLevelScene(d + b.pLevelNum),
				_czc.push(["_trackEvent", "點擊", "下一頁按鈕", d + b.pLevelNum])
			}.bind(this));
			...
			function(d) {
				...
				a.creatNormalScene(),
				_czc.push(["_trackEvent", "點擊", "選擇關卡", this.level + 1])
			})
		}
	})
} (window.Game);
