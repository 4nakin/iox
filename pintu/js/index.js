! function(e, t) {
    e("data/jigsawGame", function(e, t, n) {
        ...
        function r(e) {
            ...
            p.dom.find(".kapics").html("第("+(p.currentGame.pic + 1) + "/" + n.pic.length+")關，本關限時 "+r.second+" 秒"), u.set(f, p.currentGame), p.jigsawGame.setConfig(r)
        ...
        p.init = function(e) {
            ...
            e.html(t), p.jigsawGame = s, p.jigsawGame.init(e), p.dom = e, p.jigsawGame.on("picOver", function(e) {
				...
				var str="恭喜您挑戰成功，共闖過了<em>"+(p.currentGame.pic+1)+"</em>關";
				if(!p.hasNextPic()){
					str="恭喜您全部通關!，共闖過了<em>"+(p.currentGame.pic+1)+"</em>關,分享到朋友炫耀一下吧!";
				}
				showGameResult(str,true,p.hasNextPic());
            }), p.jigsawGame.on("timeout", function() {
				showGameResult("挑戰失敗，共闖過了<em>"+(p.currentGame.pic)+"</em>關",false,p.hasNextPic()); 
            })
