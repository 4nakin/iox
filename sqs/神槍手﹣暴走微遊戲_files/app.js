...
function homeScreen() {
    ...
    txt = new createjs.Text("開始遊戲", s10 + "px Arial", "#fff");
    ...
    btn.y = h70;
    btnStartGame.addChild(btn);
    txt = new createjs.Text("更多遊戲", s10 + "px Arial", "#fff");
    txt.textAlign = 'center';
    txt.textBaseline = "middle";
    txt.x = s50;
    txt.y = h70;
...
function newGame() {
    roundMsg = new createjs.Text("第 " + round + "/"+cancount+" 彈", s05 + "px Arial", "#ffffff");
    roundMsg.textAlign = 'left';
    roundMsg.x = s03;
    roundMsg.y = s03;
    stage.addChild(roundMsg);
    scoreMsg = new createjs.Text("總分: " + scoreAll + ".0", s05 + "px Arial", "#ffffff");
...
function newShot() {
    round++;
    roundMsg.text = "第 " + round + "/"+cancount+" 彈";
    shot = false;
    warning = true;
    miss = false;
...
function bigBall() {
    ...
    scoreAll += score;
    scoreMsg.text = "總分: " + Math.floor(scoreAll / cancount) + "." + scoreAll % cancount;
    ...
    oneShotScore.y = s15;
    if (warning == false) {
        warningMsg = new createjs.Text("(越線環數減半)", s05 + "px Arial", "#fff");
...
function checkGame() {
    ...
    resultScore = new createjs.Text("總分: " + Math.floor(scoreAll / cancount) + "." + scoreAll % cancount + " , 最准: " + Math.floor(maxScore / cancount) + "." + maxScore % cancount, s10 + "px Arial", "#ff0");
    resultScore.textAlign = 'center';
    resultScore.x = screenWidth / 2;
    resultScore.y = 0.5 * screenHeight;
    stage.addChild(resultScore);
    resultMax = new createjs.Text("記錄:  " + Math.floor(hisScore / cancount) + "." + hisScore % cancount + " , " + Math.floor(hisMax / cancount) + "." + hisMax % cancount, s10 * .8 + "px Arial", "#fff");
    resultMax.textAlign = 'center';
    resultMax.x = screenWidth / 2;
    resultMax.y = 0.6 * screenHeight;
    stage.addChild(resultMax);
    share = new createjs.Text("截屏傳到臉書去分享吧！", s05 * 1.2 + "px Arial", "#f00");
    ...
    btn.y = h61;
    btnStartGame.addChild(btn);
    txt = new createjs.Text("重新開始", s10 + "px Arial", "#fff");
    txt.textAlign = 'center';
    txt.textBaseline = "middle";
    txt.x = s50;
    txt.y = h61;
